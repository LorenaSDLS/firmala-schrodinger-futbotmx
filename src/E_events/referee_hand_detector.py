...
def analyze_referee_hand_candidates(
    ...
    for event in candidate_events:
        ...
        for frame_index in frames_to_check:
            ...
            if source_event_type == "robot_inactive_candidate":
                refined_event_type = "red_card_robot_removed"
                robot_name = source_data.get("robot_name") or object_id
                description = f"Tarjeta roja: el arbitro retiro a {robot_name}."
            ...
            new_events.append({
                ...
                "data": {
                    ...
                    "object_id": object_id,
                    "robot_id": source_data.get("robot_id"),
                    "robot_name": source_data.get("robot_name") or object_id,
                    "team": source_data.get("team", "desconocido"),
                    "target_bbox": target_bbox,
    ...
    # Un mismo retiro se inspecciona en varios cuadros. Conserva solamente
    # la detección de mano más fuerte por evento/objeto para evitar tres
    # tarjetas rojas duplicadas alrededor del mismo instante.
    deduplicated_events: dict[tuple[str, str, int], dict[str, Any]] = {}
    for candidate_event in new_events:
        data = candidate_event.get("data", {})
        key = (
            str(candidate_event.get("event_type", "")),
            str(data.get("object_id", "unknown")),
            int(round(float(candidate_event.get("timestamp_seconds", 0.0)) * 2.0)),
        )
        detections = data.get("hand_detections") or []
        strength = max((float(item.get("confidence", 0.0)) for item in detections), default=0.0)
        current = deduplicated_events.get(key)
        if current is None:
            candidate_event["confidence"] = round(strength, 4)
            deduplicated_events[key] = candidate_event
            continue
        current_strength = float(current.get("confidence", 0.0))
        if strength > current_strength:
            candidate_event["confidence"] = round(strength, 4)
            deduplicated_events[key] = candidate_event
    new_events = list(deduplicated_events.values())
        ...
        if frame_record and frame_index % frame_stride == 0:
            ...
            for target in targets:
                ...
                if target["target_type"] == "robot":
                    ...
                    if was_grabbed:
                        event_type = "red_card_robot_removed"
                        description = f"Tarjeta roja: el arbitro retiro a {target['target_id']}."

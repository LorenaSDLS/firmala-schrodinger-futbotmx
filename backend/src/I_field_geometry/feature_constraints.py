...
def homography_from_semantic_lines(
    ...
    if abs(float(cv2.contourArea(cv2.convexHull(image_array)))) < 1.0:
        return None
    image_to_field, _ = cv2.findHomography(image_array, field_array, method=0)
    if image_to_field is None or not np.isfinite(image_to_field).all():
        return None
    try:
        field_to_image = np.linalg.inv(image_to_field)
    except np.linalg.LinAlgError:
        return None
    return image_to_field / image_to_field[2, 2], field_to_image / field_to_image[2, 2]

...
class FieldGeometryResult:
    ...
    physical_boundary_scores: dict[str, float] = field(default_factory=dict)
    pose_admission_state: str = "none"
    pose_candidate_streak: int = 0
    registration_stale_frames: int = 0
    local_lock_active: bool = False
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "reverse_template_score": round(float(self.reverse_template_score), 6),
            "boundary_alignment_score": round(float(self.boundary_alignment_score), 6),
            "candidate_margin": round(float(self.candidate_margin), 6),
            "temporal_evidence_frames": int(self.temporal_evidence_frames),
            "marking_pixel_fraction": round(float(self.marking_pixel_fraction), 8),
            "candidate_count": int(self.candidate_count),
            "physical_boundary_score": round(float(self.physical_boundary_score), 6),
            "physical_boundary_count": int(self.physical_boundary_count),
            "physical_boundary_scores": {
                key: round(float(value), 6)
                for key, value in self.physical_boundary_scores.items()
            },
            "pose_admission_state": str(self.pose_admission_state),
            "pose_candidate_streak": int(self.pose_candidate_streak),
            "registration_stale_frames": int(self.registration_stale_frames),
            "local_lock_active": bool(self.local_lock_active),
...
class FieldGeometryEstimator:
    ...
    def __init__(
        ...
        self.reference_marking_accumulator = np.zeros(
            (self.frame_height, self.frame_width), dtype=np.float32
        )
        self.reference_marking_observations = np.zeros(
            (self.frame_height, self.frame_width), dtype=np.float32
        )
        self.temporal_evidence_frames = 0
        self.registration_stale_frames = 0
        self.local_lock_source = "none"
        self.pending_global_reference_corners: np.ndarray | None = None
        self.pending_global_streak = 0
        self.pending_global_score = 0.0
        self.global_confirmation_frames = 3
    ...
    def _local_result(
        ...
        pose_admission_state: str = "local",
        pose_candidate_streak: int = 0,
        ...
        result = FieldGeometryResult(
            ...
            reverse_template_score=float(reverse_template_score),
            boundary_alignment_score=float(boundary_alignment_score),
            candidate_margin=float(candidate_margin),
            temporal_evidence_frames=int(temporal_evidence_frames),
            marking_pixel_fraction=float(marking_pixel_fraction),
            ...
            pose_admission_state=str(pose_admission_state),
            pose_candidate_streak=int(pose_candidate_streak),
            registration_stale_frames=int(self.registration_stale_frames),
            local_lock_active=bool(self.reference_to_local is not None),
            field_width=self.field_width,
            field_height=self.field_height,
        )
    ...
    def _trusted_result_from_corners(
        ...
        result = FieldGeometryResult(
            ...
            reverse_template_score=float(self.last_result.reverse_template_score),
            boundary_alignment_score=float(self.last_result.boundary_alignment_score),
            candidate_margin=float(self.last_result.candidate_margin),
            temporal_evidence_frames=int(self.temporal_evidence_frames),
            marking_pixel_fraction=float(self.last_result.marking_pixel_fraction),
            candidate_count=int(self.last_result.candidate_count),
            physical_boundary_score=float(self.last_result.physical_boundary_score),
            physical_boundary_count=int(self.last_result.physical_boundary_count),
            physical_boundary_scores=dict(self.last_result.physical_boundary_scores),
            pose_admission_state="global_propagated" if not measured else "global_locked",
            pose_candidate_streak=int(self.pending_global_streak),
            registration_stale_frames=int(self.registration_stale_frames),
            local_lock_active=bool(self.reference_to_local is not None),
            field_width=self.field_width,
            field_height=self.field_height,
    ...
    def _activate_manual_calibration(
        ...
        if not calibration.is_complete:
            # A partial assisted calibration is authoritative for orientation,
            # but never for global coordinates.  Lock it in the camera-reference
            # frame so later automatic line choices cannot rotate or translate
            # the local coordinate system every few frames.
            local = calibration.local_homography_image_to_local
            if local is None:
                return None
            local = np.asarray(local, dtype=np.float64).reshape(3, 3)
            self.reference_to_local = local @ inverse_registration
            self.last_local_homography = local.copy()
            self.local_lock_source = "calibracion_asistida_local_v10"
            return self._local_result(
                source=self.local_lock_source,
                coverage=float(self.last_result.mask_coverage),
                local_homography=local,
                confidence=0.49,
                manual_line_score=1.0,
                hard_anchor_score=1.0,
                hard_anchor_count=int(calibration.feature_count),
                feature_match_score=1.0,
                feature_match_count=int(calibration.feature_count),
                feature_matches={name: 1.0 for name in calibration.semantic_segments},
                pose_admission_state="local_locked_manual",
                measured=True,
                propagated=False,
            )
    ...
    @staticmethod
    def _project_corners(corners: np.ndarray, homography: np.ndarray) -> np.ndarray | None:
        try:
    ...
    def _local_from_lock(self, current_to_reference: np.ndarray | None) -> np.ndarray | None:
        if self.reference_to_local is None or current_to_reference is None:
            return None
        value = np.asarray(current_to_reference, dtype=np.float64)
        if value.shape != (3, 3) or not np.isfinite(value).all():
            return None
        local = self.reference_to_local @ value
        return local if np.isfinite(local).all() else None

    def _lock_local_candidate(
        self,
        local_homography: np.ndarray | None,
        current_to_reference: np.ndarray | None,
        source: str,
        ...
        if local_homography is None:
            return self._local_from_lock(current_to_reference)
        candidate = np.asarray(local_homography, dtype=np.float64).reshape(3, 3)
        if not np.isfinite(candidate).all():
            return self._local_from_lock(current_to_reference)
        if self.reference_to_local is None or force:
            registration = (
                np.asarray(current_to_reference, dtype=np.float64)
                if current_to_reference is not None
                else np.eye(3, dtype=np.float64)
            )
            inverse = self._safe_inverse(registration)
            if inverse is not None:
                self.reference_to_local = candidate @ inverse
                self.local_lock_source = str(source)
                self.last_local_homography = candidate.copy()
                return candidate
        locked = self._local_from_lock(current_to_reference)
        if locked is not None:
            self.last_local_homography = locked.copy()
        return locked

    def _candidate_in_reference(
        self,
        corners: np.ndarray,
        current_to_reference: np.ndarray | None,
    ) -> np.ndarray | None:
        ...
        if registration.shape != (3, 3) or not np.isfinite(registration).all():
            return None
        return self._project_corners(corners, registration)

    def _reference_candidates_agree(
        self,
        first: np.ndarray,
        second: np.ndarray,
    ) -> bool:
        ...
        displacement = float(np.mean(np.linalg.norm(first - second, axis=1)))
        first_axis = 0.5 * (first[0] + first[3]) - 0.5 * (first[1] + first[2])
        second_axis = 0.5 * (second[0] + second[3]) - 0.5 * (second[1] + second[2])
        first_angle = float(np.arctan2(first_axis[1], first_axis[0]))
        second_angle = float(np.arctan2(second_axis[1], second_axis[0]))
        angle_delta = abs(
            float(
                np.arctan2(
        ...
        first_edges = np.linalg.norm(np.roll(first, -1, axis=0) - first, axis=1)
        second_edges = np.linalg.norm(np.roll(second, -1, axis=0) - second, axis=1)
        ratios = second_edges / np.maximum(first_edges, 1.0)
        return bool(
            displacement <= 0.045 * diagonal
            and angle_delta <= np.deg2rad(7.0)
            and np.all((ratios >= 0.72) & (ratios <= 1.38))
        )

    def _update_global_confirmation(
        self,
        corners: np.ndarray,
        current_to_reference: np.ndarray | None,
        score: float,
    ) -> bool:
        reference_corners = self._candidate_in_reference(corners, current_to_reference)
        if reference_corners is None:
            self.pending_global_reference_corners = None
            self.pending_global_streak = 0
            self.pending_global_score = 0.0
            return False
        if (
            self.pending_global_reference_corners is not None
            and self._reference_candidates_agree(
                self.pending_global_reference_corners, reference_corners
            )
        ):
            self.pending_global_reference_corners = (
                0.78 * self.pending_global_reference_corners
                + 0.22 * reference_corners
            ).astype(np.float32)
            self.pending_global_streak += 1
            self.pending_global_score = max(float(score), 0.85 * self.pending_global_score)
        else:
            self.pending_global_reference_corners = reference_corners.astype(np.float32)
            self.pending_global_streak = 1
            self.pending_global_score = float(score)
        return self.pending_global_streak >= self.global_confirmation_frames

    def _clear_global_confirmation(self) -> None:
        self.pending_global_reference_corners = None
        self.pending_global_streak = 0
        self.pending_global_score = 0.0
    ...
    def _update_temporal_marking_evidence(
        self,
        frame: np.ndarray,
        field_mask: np.ndarray,
        exclusion_boxes: Iterable[list[float]] | None,
        current_to_reference: np.ndarray | None,
        registration_quality: float | None,
        registration_updated: bool | None,
    ) -> np.ndarray | None:
        """Build a decaying temporal consensus of static field paint.

        V9 used a pixelwise maximum, so a single bad alignment remained in the
        map for the rest of the video and later acted as false confirmation.
        V10 stores decaying hit and observation counts. A transient marking can
        survive one missed frame, but it disappears when subsequent valid views
        disagree with it.
        """
        try:
            current_marking = self.template_registrar.extract_marking_mask(
                frame, field_mask, exclusion_boxes
            )
        except Exception:
            current_marking = None
        if current_to_reference is None:
            return current_marking

        matrix = np.asarray(current_to_reference, dtype=np.float64)
        if matrix.shape != (3, 3) or not np.isfinite(matrix).all():
            return current_marking
        quality = 1.0 if registration_quality is None else float(registration_quality)
        may_update = bool(
            current_marking is not None
            and (
                self.temporal_evidence_frames == 0
                or (quality >= 0.20 and registration_updated is not False)
            )
        )
        if may_update:
            marking_for_vote = cv2.dilate(
                (current_marking > 0).astype(np.uint8) * 255,
                np.ones((3, 3), np.uint8),
            )
            visible_region = cv2.dilate(
                (field_mask > 0).astype(np.uint8) * 255,
            ...
            decay = 0.93
            self.reference_marking_accumulator *= decay
            self.reference_marking_observations *= decay
            self.reference_marking_accumulator += warped
            self.reference_marking_observations += warped_visible
            np.clip(self.reference_marking_accumulator, 0.0, 12.0, out=self.reference_marking_accumulator)
            np.clip(self.reference_marking_observations, 0.0, 12.0, out=self.reference_marking_observations)
            self.temporal_evidence_frames = min(60, self.temporal_evidence_frames + 1)

        inverse = self._safe_inverse(matrix)
        if (
            inverse is None
            or self.temporal_evidence_frames < 2
            or self.registration_stale_frames > 2
        ):
            return current_marking

        observations = np.maximum(self.reference_marking_observations, 1e-6)
        consensus_ratio = self.reference_marking_accumulator / observations
        reference_binary = (
            (self.reference_marking_observations >= 1.50)
            & (self.reference_marking_accumulator >= 0.75)
            & (consensus_ratio >= 0.44)
        ).astype(np.uint8) * 255
        reference_binary = cv2.morphologyEx(
            reference_binary,
            cv2.MORPH_OPEN,
            np.ones((2, 2), np.uint8),
        )
        prior_current = cv2.warpPerspective(
            reference_binary,
            inverse,
            (self.frame_width, self.frame_height),
            flags=cv2.INTER_NEAREST,
        ...
        return cv2.max(current_marking, prior_current)
    ...
    def update(
        ...
        registration_updated: bool | None = None,
    ) -> FieldGeometryResult:
        self.frames_since_measurement += 1
        quality = 0.0 if registration_quality is None else float(registration_quality)
        if frame_index in {None, 0} and self.frames_since_measurement >= 10_000:
            self.registration_stale_frames = 0
        elif registration_updated is True and quality >= 0.18:
            self.registration_stale_frames = 0
        else:
            self.registration_stale_frames = min(10_000, self.registration_stale_frames + 1)
        ...
        if activated is not None:
            return activated
        waiting_for_partial_manual = bool(
            self.manual_calibration is not None
            and not self.manual_calibration.is_complete
            and not self.manual_calibration_active
            and frame_index is not None
            and frame_index < self.manual_calibration.source_frame_index
        )
        predicted_corners = self._predicted_corners(current_to_reference)

        # Tracking and relocalization are separate operations in V10. Between
        # segmentation measurements, a previously admitted global pose is only
        # transported by camera registration; no legacy mask bootstrap is
        # allowed to redefine its state or scope.
        if (
            segmentation is None
            and self.last_result.trusted
            and self.last_result.geometry_state == "global"
            and predicted_corners is not None
        ):
            if self.registration_stale_frames <= 6 and self.frames_since_measurement <= 48:
                homography = cv2.getPerspectiveTransform(
                ...
                confidence = max(
                    0.30,
                    float(self.last_result.confidence)
                    * (0.988 if self.registration_stale_frames == 0 else 0.965),
                )
                return self._trusted_result_from_corners(
                    predicted_corners,
                    homography,
                    source=(
                        "calibracion_asistida_propagada"
                        if self.manual_calibration_active
                        and self.manual_calibration is not None
                        and self.manual_calibration.is_complete
                        else "global_propagada_v10"
                    ),
                    measured=False,
                    confidence=confidence,
                    coverage=float(self.last_result.mask_coverage),
                )
            locked_local = self._local_from_lock(current_to_reference)
            return self._local_result(
                source="global_suspendida_registro_incierto_v10",
                coverage=float(self.last_result.mask_coverage),
                local_homography=locked_local,
                confidence=0.20,
                pose_admission_state="global_suspended",
                measured=False,
                propagated=True,
            )
        ...
        if (
            ...
            local_h = self._local_from_lock(current_to_reference)
            if local_h is None:
                self.last_result = self._empty_result("orientacion_local_sin_registro", 0.0)
                return self.last_result
            self.last_local_homography = local_h.copy()
            return self._local_result(
                source="orientacion_local_bloqueada_v10",
                coverage=float(self.last_result.mask_coverage),
                ...
                physical_boundary_count=int(self.last_result.physical_boundary_count),
                physical_boundary_scores=dict(self.last_result.physical_boundary_scores),
                pose_admission_state="local_locked",
                pose_candidate_streak=int(self.pending_global_streak),
        ...
        # V9 unlocks global coordinates only after dense template evidence,
        # distinct line-family support and ambiguity checks agree. Rails and
        # the surface mask remain local evidence and cannot invent location.
        ...
        if cleaned is not None and frame is not None:
            temporal_marking_mask = self._update_temporal_marking_evidence(
                frame=frame,
                field_mask=cleaned,
            ...
            # FieldGeometry stores [far-left, far-right, near-right,
            # near-left]; the template registrar uses [near-left, far-left,
            # far-right, near-right]. Keep temporal comparisons in one order.
            predicted_template_corners = (
                None
                if predicted_corners is None
                else np.asarray(predicted_corners, dtype=np.float32)[[3, 0, 1, 2]]
            )
            template_result = self.template_registrar.register(
                ...
                exclusion_boxes=exclusion_boxes,
                predicted_corners=predicted_template_corners,
                semantic_segments=manual_segments,
                temporal_marking_mask=temporal_marking_mask,
                temporal_evidence_frames=self.temporal_evidence_frames,
            )
            global_candidate_pending = False
            if (
                ...
                admitted = False
                admission_state = "global_candidate_pending"
                if existing_global:
                    diagonal = float(np.hypot(self.frame_width, self.frame_height))
                    discrepancy = float(
                        np.mean(
                    ...
                    if discrepancy <= 0.055 * diagonal:
                        admitted = True
                        admission_state = "global_revalidated"
                        self._clear_global_confirmation()
                    else:
                        admitted = self._update_global_confirmation(
                            corners,
                            current_to_reference,
                            float(template_result.confidence),
                        )
                        admission_state = (
                            "global_relocalized"
                            if admitted
                            else "global_relocalization_pending"
                        )
                else:
                    admitted = self._update_global_confirmation(
                        corners,
                    ...
                    admission_state = (
                        "global_acquired"
                        if admitted
                        else "global_acquisition_pending"
                    )
                ...
                if admitted:
                    ...
                    if inverse is not None:
                        ...
                        self.last_corners = corners.copy()
                        confirmed_streak = int(self.pending_global_streak)
                        self.last_result = FieldGeometryResult(
                            ...
                            homography_field_to_image=inverse,
                            mask_coverage=coverage,
                            source=f"{template_result.source}_confirmada_v10",
                            line_support={
                                "physical_boundaries": int(template_result.physical_boundary_count),
                                "boundary_score": int(round(100 * template_result.physical_boundary_score)),
                            },
                            side_visible={side: False for side in SIDE_NAMES},
                            side_status={side: "pose_bloqueada_v10" for side in SIDE_NAMES},
                            ...
                            reverse_template_score=float(template_result.reverse_template_score),
                            boundary_alignment_score=float(template_result.boundary_alignment_score),
                            candidate_margin=float(template_result.candidate_margin),
                            temporal_evidence_frames=int(template_result.temporal_evidence_frames),
                            marking_pixel_fraction=float(template_result.marking_pixel_fraction),
                            candidate_count=int(template_result.candidate_count),
                            physical_boundary_score=float(template_result.physical_boundary_score),
                            physical_boundary_count=int(template_result.physical_boundary_count),
                            physical_boundary_scores=dict(template_result.physical_boundary_scores or {}),
                            pose_admission_state=admission_state,
                            pose_candidate_streak=confirmed_streak,
                            registration_stale_frames=int(self.registration_stale_frames),
                            local_lock_active=bool(self.reference_to_local is not None),
                            field_width=self.field_width,
                            field_height=self.field_height,
                    ...
                    global_candidate_pending = True

            # A previously admitted global solution may be transported through a
            # short interval, but an isolated relocalization hypothesis cannot
            # overwrite it. The state remains explicitly GLOBAL while tracking.
            if (
                self.last_result.trusted
                and self.last_result.geometry_state == "global"
                and predicted_corners is not None
                and self.registration_stale_frames <= 6
            ):
                homography = cv2.getPerspectiveTransform(
                    np.asarray(predicted_corners, dtype=np.float32), self.canonical_corners
                )
                confidence = max(0.30, float(self.last_result.confidence) * 0.978)
                propagated = self._trusted_result_from_corners(
                    predicted_corners,
                    homography,
                    source=(
                        "global_relocalizacion_pendiente_v10"
                        if global_candidate_pending
                        else "global_propagada_sin_anclas_v10"
                    ),
                    measured=False,
                    confidence=confidence,
                    coverage=coverage,
                ...
                propagated.reverse_template_score = float(template_result.reverse_template_score)
                propagated.boundary_alignment_score = float(template_result.boundary_alignment_score)
                propagated.candidate_margin = float(template_result.candidate_margin)
                propagated.temporal_evidence_frames = int(template_result.temporal_evidence_frames)
                propagated.marking_pixel_fraction = float(template_result.marking_pixel_fraction)
                propagated.candidate_count = int(template_result.candidate_count)
                propagated.physical_boundary_score = float(template_result.physical_boundary_score)
                propagated.physical_boundary_count = int(template_result.physical_boundary_count)
                propagated.physical_boundary_scores = dict(template_result.physical_boundary_scores or {})
                propagated.pose_admission_state = (
                    "global_relocalization_pending"
                    if global_candidate_pending
                    else "global_propagated"
                )
                propagated.pose_candidate_streak = int(self.pending_global_streak)
                return propagated
            ...
            candidate_local = (
                None
                if waiting_for_partial_manual
                else template_result.local_homography_image_to_local
            )
            locked_local = self._lock_local_candidate(
                candidate_local,
                current_to_reference,
                source=template_result.source,
            )
            local_source = (
                "esperando_calibracion_asistida_v10"
                if waiting_for_partial_manual
                else f"candidata_global_pendiente_v10_{self.pending_global_streak}de{self.global_confirmation_frames}"
                if global_candidate_pending
                else "orientacion_local_bloqueada_v10"
                if locked_local is not None
                else template_result.source
            )
            return self._local_result(
                source=local_source,
                coverage=coverage,
                ...
                physical_boundary_count=int(template_result.physical_boundary_count),
                physical_boundary_scores=dict(template_result.physical_boundary_scores or {}),
                pose_admission_state=(
                    "global_acquisition_pending"
                    if global_candidate_pending
                    else "local_locked"
                    if locked_local is not None
                    else "surface_only"
    ...
    def annotate_detection(self, detection: dict[str, Any]) -> dict[str, Any]:
        ...
        if group == "robot":
            ...
            # The image centre of a spherical ball is above the carpet plane.
            # The lower contour is the planar contact proxy compatible with the
            # field homography and avoids a systematic camera-height bias.
            anchor = ((x1 + x2) * 0.5, y2)
            anchor_type = "bottom_contact"
        else:
        ...
        if transformed is not None:
            ...
            # V11 is fail-closed: a visually supported point that maps outside
            # the metric carpet is evidence of a poor anchor or an occlusion,
            # not a valid competition coordinate. Goals are allowed to extend
            # outside the carpet plane, but robots and the ball are not.
            if group in {"robot", "ball"} and not inside_surface:
                result["field_transform_valid"] = False
                result["field_transform_extrapolated"] = True
...
def draw_field_geometry_overlay(
    ...
    if geometry.valid and geometry.homography_field_to_image is not None:
        template = build_template_points(density=300)
        metric_points = template.points.astype(np.float32).copy()
        metric_points[:, 0] *= float(geometry.field_width)
        metric_points[:, 1] *= float(geometry.field_height)
        projected_template = cv2.perspectiveTransform(
            metric_points.reshape(1, -1, 2),
            geometry.homography_field_to_image,
        ).reshape(-1, 2)
        if geometry.corners_image is not None:
            fill = output.copy()
            cv2.fillConvexPoly(
                fill,
                np.rint(geometry.corners_image).astype(np.int32),
                (50, 220, 210),
                cv2.LINE_AA,
            )
            output = cv2.addWeighted(fill, 0.10, output, 0.90, 0.0)
        group_colors = {
            0: (40, 255, 255),
            1: (235, 235, 235),
            2: (60, 245, 255),
        ...
        for index in range(1, len(projected_template)):
            if template.groups[index] != template.groups[index - 1]:
                continue
            first, second = projected_template[index - 1], projected_template[index]
            if not np.isfinite(first).all() or not np.isfinite(second).all():
                continue
            if float(np.linalg.norm(second - first)) > 0.25 * max(output.shape[:2]):
                continue
            cv2.line(
                output,
                tuple(np.rint(first).astype(int)),
                tuple(np.rint(second).astype(int)),
                group_colors.get(int(template.groups[index]), (255, 255, 255)),
                3 if int(template.groups[index]) == 0 else 2,
                cv2.LINE_AA,
            )
    ...
    is_hologram = str(geometry.source).startswith("holograma_v11")
    diagnostic_text = (
        f"V11 holograma estado={geometry.pose_admission_state} | "
        f"desacuerdo={geometry.feature_matches.get('anchor_disagreement_px', 0.0):.1f}px | "
        f"oclusion/espera={int(geometry.feature_matches.get('stale_gap_frames', 0.0))}f | "
        f"registro={geometry.feature_match_score:.2f}"
        if is_hologram
        else (
            f"V10 estable inv {geometry.reverse_template_score:.2f} | limite {geometry.boundary_alignment_score:.2f} | "
            f"margen {geometry.candidate_margin:.3f} | temporal {geometry.temporal_evidence_frames} | "
            f"pintura {100.0 * geometry.marking_pixel_fraction:.2f}% | hipotesis {geometry.candidate_count}"
        )
    )
    cv2.putText(
        output,
...
def draw_field_evidence_debug(
    frame: np.ndarray,
    geometry: FieldGeometryResult,
    debug: dict[str, Any] | None,
) -> np.ndarray:
    """Render V10 visual evidence and hypothesis search as a four-panel frame."""
    data = debug or {}
    marking = data.get("marking_mask")
    boundary = data.get("boundary_mask")
    if isinstance(marking, np.ndarray) and marking.ndim == 2:
        work_height, work_width = marking.shape[:2]
    elif isinstance(boundary, np.ndarray) and boundary.ndim == 2:
        work_height, work_width = boundary.shape[:2]
    else:
        work_height, work_width = frame.shape[:2]
    base = cv2.resize(frame, (work_width, work_height), interpolation=cv2.INTER_AREA)

    def gray_panel(value: Any) -> np.ndarray:
        if not isinstance(value, np.ndarray) or value.ndim != 2:
            return np.zeros_like(base)
        image = value
        if image.shape != (work_height, work_width):
            image = cv2.resize(image, (work_width, work_height), interpolation=cv2.INTER_NEAREST)
    ...
    lines_panel = base.copy()
    for raw in data.get("marking_lines", []) or []:
        segment = np.asarray(raw, dtype=np.float64).reshape(2, 2)
        points = np.rint(segment).astype(np.int32)
        cv2.line(lines_panel, tuple(points[0]), tuple(points[1]), (0, 235, 255), 2, cv2.LINE_AA)
    for raw in data.get("side_lines", []) or []:
        segment = np.asarray(raw, dtype=np.float64).reshape(2, 2)
        points = np.rint(segment).astype(np.int32)
        cv2.line(lines_panel, tuple(points[0]), tuple(points[1]), (255, 210, 0), 2, cv2.LINE_AA)

    hypotheses_panel = base.copy()
    candidates = data.get("candidate_corners", []) or []
    for index, raw in enumerate(candidates[:8]):
        corners = np.asarray(raw, dtype=np.float64).reshape(4, 2)
        if not np.isfinite(corners).all():
            continue
        color = (70 + 18 * min(index, 7), 90, 235 - 18 * min(index, 7))
        cv2.polylines(
            hypotheses_panel,
    ...
    best = data.get("best_corners")
    if isinstance(best, np.ndarray) and np.asarray(best).size == 8:
        best_corners = np.asarray(best, dtype=np.float64).reshape(4, 2)
        if np.isfinite(best_corners).all():
            cv2.polylines(
                hypotheses_panel,
                [np.rint(best_corners).astype(np.int32)],
                True,
                (40, 255, 40) if geometry.trusted else (0, 190, 255),
                3,
                cv2.LINE_AA,
            )

    panels = [lines_panel, gray_panel(marking), gray_panel(boundary), hypotheses_panel]
    labels = [
        "LINEAS: AMARILLO=PINTURA, CYAN=BORDE",
        "MASCARA ADAPTATIVA DE PINTURA",
        "BORDE FISICO / RIEL",
        "HIPOTESIS (VERDE=ACEPTADA)",
    ]
    half_width = max(1, frame.shape[1] // 2)
    half_height = max(1, frame.shape[0] // 2)
    resized: list[np.ndarray] = []
    for panel, label in zip(panels, labels):
        item = cv2.resize(panel, (half_width, half_height), interpolation=cv2.INTER_AREA)
        cv2.rectangle(item, (0, 0), (half_width - 1, 30), (20, 20, 20), cv2.FILLED)
        cv2.putText(item, label, (8, 21), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (245, 245, 245), 1, cv2.LINE_AA)
        resized.append(item)
    top = np.hstack(resized[:2])
    bottom = np.hstack(resized[2:])
    output = np.vstack([top, bottom])
    output = cv2.resize(output, (frame.shape[1], frame.shape[0]), interpolation=cv2.INTER_AREA)
    footer = (
        f"estado={geometry.geometry_state} conf={geometry.confidence:.2f} "
        f"denso={geometry.reverse_template_score:.2f} margen={geometry.candidate_margin:.3f} "
        f"temporal={geometry.temporal_evidence_frames} fuente={data.get('source', geometry.source)}"
    )
    cv2.rectangle(output, (0, output.shape[0] - 34), (output.shape[1] - 1, output.shape[0] - 1), (18, 18, 18), cv2.FILLED)
    cv2.putText(output, footer[:180], (10, output.shape[0] - 11), cv2.FONT_HERSHEY_SIMPLEX, 0.46, (245, 245, 245), 1, cv2.LINE_AA)
    return output
...
def _render_local_rectification(
    ...
    cv2.putText(canvas, f"V10 {state_text} - posicion global desconocida", (14, 31), cv2.FONT_HERSHEY_SIMPLEX, 0.56, (0, 235, 255), 2, cv2.LINE_AA)
...
def render_rectified_debug(
    ...
    """Render V10 progressive geometry without inventing a global field."""
    ...
    cv2.putText(
        canvas,
        (
            f"{'V11 HOLOGRAMA' if str(geometry.source).startswith('holograma_v11') else 'V10 GLOBAL'} {status} / {trust} | visible {support_fraction * 100:.1f}% | "

...
def reconstruct_physical_identities(
        ...
        for record_index, record in enumerate(records):
            ...
            for detection_index, detection in enumerate(record.get("detections", [])):
                ...
                if active is not None and len(active.feature_views) >= config.maximum_samples_per_tracklet:
                    # Continue checking often enough to catch an ID swap after
                    # a late crossing. V11 sampled every 32 frames here and
                    # could miss the complete exchange.
                    sample_interval *= 2
                if bool(detection.get("association_ambiguous", False)):
                    # Do not trust the contaminated crossing crop itself, but
                    # force nearby frames to be inspected frequently.
                    sample_interval = max(1, sample_interval // 2)
    ...
    for record in records:
        ...
        unresolved_robots: list[dict[str, Any]] = []
        others: list[dict[str, Any]] = []
        for item in rewritten:
            if str(item.get("class_group", "")).lower() != "robot":
                others.append(item)
                continue
            physical_id_raw = item.get("physical_robot_id")
            if physical_id_raw is None:
                # Some short/occluded tracklets intentionally remain
                # unresolved.  V11 used to preserve them above and then crash
                # here by assuming every robot had a physical ID.
                item["physical_robot_id"] = None
                item["identity_resolved_offline"] = False
                unresolved_robots.append(item)
                continue
            try:
                physical_id = int(physical_id_raw)
        ...
        record["detections"] = others + list(robots_by_id.values()) + unresolved_robots
